...
var Bookmarks = require("../dist/index.js");
...
describe("L.Bookmarks.FormPopup", () => {
  ...
  var map = new L.Map(container, {}).setView([22.267, 114.188], 13);
  ...
  map.whenReady(function () {
    ...
    it("constructor", () => {
      ...
      assert.ok(
        coord.equals(bookmarksControl._marker.getLatLng()),
        "marker on right coordinate"
      );
      assert.ok(
        coord.equals(bookmarksControl._popup.getLatLng()),
        "popup on right coordinate"
      );
      bookmarksControl._popup.close();
    });
    ...
    it("add bookmark", () => {
      ...
      assert.equal(
        bookmarksControl._popup._bookmark,
        bookmark,
        "popup linked to bookmark"
      );
      assert.ok(
        bookmarksControl._list.querySelector("[data-id='" + id + "']"),
        "in list"
      );
      assert.equal(
        bookmarksControl.getData().filter((b) => b.id === id).length,
        1,
        "in data"
      );
    ...
    it("remove bookmark", () => {
      ...
      map.fire("bookmark:add", { data: bookmark });
      assert.notEqual(
        bookmarksControl.getData().indexOf(bookmark),
        -1,
        "stored"
      );
    ...
    it("edit bookmark", () => {
      ...
      var input =
        bookmarksControl._popup._contentNode.querySelector("input[type=text]");
      var suffix = " " + uid();
      ...
      map.once("bookmark:saved", (evt) => {
        ...
        assert.ok(
          newCoord.lng === item.latlng[1] && newCoord.lat === item.latlng[0],
          "new coord saved"
        );
      });
